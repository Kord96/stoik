...
def open_graph(*db_paths: str) -> duckdb.DuckDBPyConnection:
    ...
    Local: ATTACHes DuckDB files under aliases (e.g., ``domain.domain``).
    MinIO: creates views with aliased table names (e.g., ``base_domain.domain``).
    """
    if SNAPSHOT_BACKEND == 'minio':
        return _open_graph_minio(*db_paths)
    return _open_graph_local(*db_paths)


def _open_graph_local(*db_paths: str) -> duckdb.DuckDBPyConnection:
    conn = duckdb.connect(':memory:')
...
def _open_graph_minio(*db_paths: str) -> duckdb.DuckDBPyConnection:
    conn = duckdb.connect(':memory:')
    configure_s3(conn)
    ...
    for db_path in db_paths:
        ...
        # Create a schema (like ATTACH alias) and views for each table
        conn.execute(f'CREATE SCHEMA IF NOT EXISTS "{stem}"')
        for (filepath,) in files:
            table = Path(filepath).stem
            conn.execute(
                f'CREATE VIEW "{stem}"."{table}" AS '
                f"SELECT * FROM read_parquet('{filepath}')"
            )
